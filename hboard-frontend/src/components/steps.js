...
export class CollinearityDetectionStep extends StepWithStdConfigAndStatusAndFeatureCard  {
    ...
    render() {
        ...
        let dataSource;
        if (stepData.status === StepStatus.Finish) {
            const unselectedFeatures = stepData.extension.unselected_features;
            dataSource = []
            let featureIndex = 0 ;
            for(const removedFeature in unselectedFeatures){
                if(unselectedFeatures.hasOwnProperty(removedFeature)){
                    const reservedFeature = unselectedFeatures[removedFeature].reserved
                    dataSource.push({
                        key: featureIndex,
                        removed: removedFeature,
                        reserved: reservedFeature,
                    })
                }
...
export class DriftDetectionStep extends StepWithStdConfigAndStatusAndFeatureCard {
    ...
    render() {
        const stepData = this.stepData;
        const unselectedFeatures = this.stepData.extension.unselected_features;

        let overVariableThresholdFeaturesDataSource;
        let overThresholdFeatures;

        if (stepData.status === StepStatus.Finish) {
            overVariableThresholdFeaturesDataSource = unselectedFeatures.over_variable_threshold.map((value, index, arr) => {
                return {
                    key: index,
                    feature: value[0],
                    score: formatFloat(value[1]),
                }
            });
            overThresholdFeatures = unselectedFeatures.over_threshold;
        } else {
            overVariableThresholdFeaturesDataSource = [];
            overThresholdFeatures = [];
        }
                ...
                <Row gutter={[2, 2]}>
                    <Col span={24}>
                        <Card title={removeByEvaluateResultTitle} bordered={false} style={{width: '100%'}}
                              size={COMPONENT_SIZE}>
                            <Table dataSource={overVariableThresholdFeaturesDataSource}
...
export class EnsembleStep extends StepWithStdConfigAndStatusAndFeatureCard {
    ...
    render() {
        const stepData = this.stepData;
        const estimatorsData = this.stepData.extension.estimators;

        console.info("estimatorsData")
        console.info(estimatorsData)

        const isEstimatorDataNotEmpty = (d) => {
        ...
        const getLiftEchartOpts = () => {
            ...
            if (stepData.status === StepStatus.Finish) {
                ...
                if(isEstimatorDataNotEmpty(estimatorsData)){
                    yLabels = Array.from({length: estimatorsData.length}, (v, k) => k);
                    scores = [ ...estimatorsData.map( v=> v.lift)]
        ...
        const getWeightsEchartOpts = () => {
            ...
            if (stepData.status === StepStatus.Finish) {
                ...
                if(isEstimatorDataNotEmpty(estimatorsData)){
                    yLabels = Array.from({length: estimatorsData.length}, (v, k) => k);
                    weights = [ ...estimatorsData.map( v=> v.weight)]
                }
            }
